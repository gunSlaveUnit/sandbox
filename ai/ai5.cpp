...
std::array<std::array<std::array<double, 2>, LAYER_DIMENSION>, LAYER_DIMENSION> W{};
...
byte recognize(bool show_guess = false) {
    ...
    for (i32 depth = 0; depth < 2; ++depth) {
        for (i32 row = 0; row < LAYER_DIMENSION; ++row)
            for (i32 column = 0; column < LAYER_DIMENSION; ++column)
                e[depth] += A[row][column] * W[row][column][depth];

        if (e[depth] > 0)
            result |= (byte) 1 << (1 - depth);
        else
            result &= ~(byte) (1 << (1 - depth));
    }
...
void teach(FigureTypes generated_figure_type, FigureTypes recognized_figure_type) {
    for (i32 depth = 0; depth < 2; ++depth)
        if ((generated_figure_type >> (1 - depth) & 1) != (recognized_figure_type >> (1 - depth) & 1))
            for (i32 row = 0; row < LAYER_DIMENSION; ++row)
                for (i32 column = 0; column < LAYER_DIMENSION; ++column)
                    if (A[row][column] == 1) {
                        if (!(generated_figure_type >> (1 - depth) & 1))
...
int main() {
    ...
    for (i32 step = 1; step <= LEARNING_STEPS_AMOUNT; ++step) {
        ...
        if (generated_figure_type == recognized_figure_type)
            ...
            teach(static_cast<FigureTypes>(generated_figure_type), static_cast<FigureTypes>(recognized_figure_type));
