const STRING_POINTS_AMOUNT = 50
const TAU = 0.1
const GRID_STEP = 1
const WAVE_SPEED = 2
const LEARNING_ACCURACY_BOUND = 0.01
const LEARNING_WEIGHT = 0.01
const AMPLITUDE_MULTIPLICITY = 3

const CANVAS_WIDTH = 1200
const CANVAS_HEIGHT = 600

let dx = (CANVAS_WIDTH - 1) / (STRING_POINTS_AMOUNT - 1)
...
var weights = Array<number>(STRING_POINTS_AMOUNT).fill(0)
var ut = Array<number>(STRING_POINTS_AMOUNT).fill(0)
var u = Array<number>(STRING_POINTS_AMOUNT).fill(0)
var un = Array<number>(STRING_POINTS_AMOUNT).fill(0)
var us = Array<number>(STRING_POINTS_AMOUNT).fill(0)
var f = Array<number>(STRING_POINTS_AMOUNT).fill(0)
us[5] = u[5] - TAU * 100
...
function new_step() {
  for (let i = 1; i < STRING_POINTS_AMOUNT - 1; i++)
  ...
  for (let j = 1; j < STRING_POINTS_AMOUNT - 1; j++) {
    for (let i = -1; i <= 1; i++)
  ...
  let dU = ut[learning_point_index] - un[learning_point_index]
...
function teach(e: number) {
  let weight_additives = Array<number>(4)
  let sign = -1
  for (let i = -1; i <= 1; i++) {
...
function draw_frame() {
  ...
  for (let i = 1; i < STRING_POINTS_AMOUNT; i++) {
    let x = i * dx
    let y = (y_c - AMPLITUDE_MULTIPLICITY * ut[i])
    context.lineTo(x, y)
  }
