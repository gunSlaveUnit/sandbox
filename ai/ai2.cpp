...
const i32 MINIMAL_FIGURE_RADIUS = 5;
    ...
    switch (figure_type) {
        ...
        case TRIANGLE:
            for (i32 i = -image_radius; i <= image_radius; ++i)
                for (i32 j = -image_radius; j < i; ++j)
                    if (i == image_radius)
                        R[i + center.row][j + center.column] = 1;
                    else if (j == -image_radius || j == i - 1)
                        R[i + center.row][j + center.column] = 1;
    }
...
FigureTypes recognize(bool show_guess = false) {
    ...
    if (show_guess) {
        ...
        if (e > 0)
            std::cout << "I think is a triangle" << std::endl;
        else
            std::cout << "I think is a circle" << std::endl;
